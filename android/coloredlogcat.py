...
# modified by James Smith, http://loopj.com to support motorola logcat output
...
MATCH_TAGS = [
    # Most android phones
    re.compile("^(?P<loglevel>[A-Z])/(?P<tag>[^\(]+)\((?P<pid>[^\)]+)\): (?P<message>.*)$"),

    # Motorola phones
    re.compile("^(?P<date>[0-9-]+)\W+(?P<time>[0-9:.]+)\W+(?P<pid>\d+)\W+(?P<ppid>\d+)\W+(?P<loglevel>[A-Z])\W+(?P<tag>[^:]+):\W+(?P<message>.*)$")
]
...
while True:
    ...
    if not match is None:
        ...
        # center process info
        if PROCESS_WIDTH > 0:
            owner = match.group("pid").strip().center(PROCESS_WIDTH)
        ...
        if not match.group("loglevel") in TAGTYPES: break
        linebuf.write(TAGTYPES[match.group("loglevel")])

        # insert line wrapping as needed
        message = indent_wrap(match.group("message"), HEADER_SIZE, WIDTH)
